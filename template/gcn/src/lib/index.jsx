...
import {
    Row, Table, Button, message,
    Form, Input, Space, Breadcrumb,
    DatePicker
} from "antd";
...
export function EditTable(props) {
    let {columns, data, editURL, tableTitle, loading, onAdd} = props;
        ...
        return (
            <div className={"clearfix export-table-title"}>
                ...
                <Space direction={"horizontal"}>
                    <Button type={"primary"} onClick={() => (onAdd && onAdd())}>新增</Button>
    ...
    let mergedColumns = [
        ...
        ...columns,
        {
            ...
            render: (text, record) => (
                <Space>
                    <Link to={`${editURL}edit/${record["key"]}`} target={"_blank"}>
                        <Button type={"link"} size={"small"}>编辑</Button>
                    </Link>
                    <Link to={`${editURL}view/${record["key"]}`} target={"_blank"}>
        ...
        return {
            ...
            sorter: (a, b) => {
                let stringA = a[v.dataIndex];
                let stringB = b[v.dataIndex];
                if (stringA < stringB) {
                    return -1;
                }
                if (stringA > stringB) {
...
export function Container(props) {
    ...
    return (
        <Form colon={false} className={"g-from"}>
...
export function DateStrPicker(props) {
    let {onChange, value, name} = props;

    const changeVal = (val, strVal) => {
        onChange(name, strVal);
    };

    return (
        <DatePicker style={{width: "100%"}} value={moment(value)} onChange={changeVal}/>
    );
}

export function DateStrRange(props) {
    let {onChange, value, name} = props;

    let [n1, n2] = name.split(",");
    let [d1, d2] = value;

    const changeVal = (momentVal, strVal) => {
        onChange(n1, strVal[0]);
        onChange(n2, strVal[1]);
    };

    return (
        <DatePicker.RangePicker value={[moment(d1), moment(d2)]} style={{width: "100%"}}
                                onChange={changeVal}/>
    );
}
